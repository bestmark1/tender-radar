...
В [`tasks/`](tasks/) лежат решения тестовых заданий по вакансии — отдельно
от продукта, фазовый цикл `SPEC_PLAN/` на них не распространяется:

| № | Задание | Стек |
|---|---|---|
| 4 | [Схема БД «Тендерная площадка»](tasks/task-04-db-schema/SOLUTION.md) — таблицы, FK, индексы, аналитические запросы, планы выполнения | PostgreSQL 16 |
| 6 | [Микросервис трекинга статусов тендеров](tasks/task-06-tender-status-api/SOLUTION.md) — эндпоинты, машина состояний, аудит изменений | FastAPI, SQLAlchemy 2.0 |

Схема из задания 4 развивает модель данных этого проекта
([`supabase/migrations/`](supabase/migrations/)) до полноценной площадки со
ставками и исполнителями; сервис из задания 6 работает поверх неё.
